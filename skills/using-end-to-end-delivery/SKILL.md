...
- **测试只走 SSH**：远端测试使用 `e2e-remote-test`（SSH 到开发机），不使用 BITS quick-run
- **不做上线后排障**：MVP 交付到 PPE 部署完成为止，用户提上线后排障需求时明确告知"当前版本暂不支持，建议走内部 oncall 流程"
...
5. ☐ 到 HARD-GATE？
6. ☐ 三活文档遵守"单一创建"？
7. ☐ MVP 范围内（不做 quick-run / 不做上线后排障）？
8. ☐ 用户得到**可执行下一步**？
