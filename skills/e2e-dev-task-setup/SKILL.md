...
description: "端到端交付的研发任务初始化 skill（职责已简化），基于方案设计阶段产出的 task.md 创建**单个** BITS 研发任务，返回任务链接。本 skill **不再拆解任务**（拆解已由 e2e-solution-design 完成并写入 task.md）。1 个需求 = 1 个 BITS task（可包含 N 个 MR）。本 skill 是写操作 skill，必须遵守 HARD-GATE：首次调用必须带 --dry-run 展示 payload，用户明确确认后才实际创建。当方案设计已完成、需要在 BITS 创建研发任务登记管理、关联需求到代码的场景必须调用此 skill。典型触发场景：'帮我创建研发任务'、'BITS 开个任务'、'绑定分支到 BITS'、'我们要开始开发'、'方案定稿了，登记任务'、'建 BITS 任务'。内部调用 bytedance-auth 确认登录 + bytedance-bits 创建任务。产出的 BITS task 链接给 e2e-code-review-loop 作为上下文引用。"
---

# E2E Dev Task Setup —— 研发任务初始化（简化版）

## 定位

端到端交付主流程的**阶段 5 入口**：基于方案设计（plan.md / task.md）在 BITS 平台登记 1 个研发任务。

**职责简化说明**：

- **以前**（v1.0）：基于 codebase-mapping 的"涉及仓库清单"推断并拆解任务
- **现在**（v2.0）：基于 task.md **直接读取**已拆好的任务清单，**只做登记**

**输入**：

- `specs/[简称]/task.md`（方案设计阶段产出）
- PRD 元信息（研发负责人、QA、需求单号等）

**输出**：BITS 上的 **1 个** dev-task ID（覆盖本次所有涉及仓库）
...
- ✅ 关联 Meego 需求单（如有）
- ❌ **不拆解任务**（task.md 已拆好）
- ❌ **不创建多个 BITS task**（1 个需求 = 1 个 BITS task）
...
- [x] `e2e-solution-design` 已产出 `specs/[简称]/task.md` 和 `plan.md`
...
**仓库清单来源**：从 task.md 的"涉及仓库/PSM"字段汇总（去重后）。
...
**每个涉及仓库一个 `--change`**。从 `task.md` 的"涉及仓库/PSM"字段**聚合去重**：

```
读 specs/[简称]/task.md 的每个任务的 "仓库/PSM" 字段
聚合去重 → 涉及仓库清单

例如 task.md 中：
- T1-T5: user.segment.api
- T6: segment.calculator.service
- T8-T9: operation.platform.web
...
- task.md 里涉及的**所有**仓库都加入 `--change`（任务已经过 solution-design 阶段筛选）
- **1 个需求 = 1 个 BITS task**（不创建多个）
